...
.select(col('value.existType'), col("value.Ch"), col("value.Incr"), col("value.zSetEntries")).createOrReplaceTempView("RedisSortedSet")
...
emailAndBirthYearDF = emailAndBirthDayStreamingDF.select("email", col("birthYear"))
...
# +--------------------+-----
emailAndBirthYearDF.writeStream.outputMode("append").format("console").start().awaitTermination()
